...
void mm_free(Bptr bp) {
    ...
    if (GET_SIZE(NEXT_BLKP(bp)) != (size_t)0 ||
        GET_BLK_ALLOC(NEXT_BLKP(bp)) != 1) { // not epilogue block

        // dbg_printf("not epilogue block \n");
        // dbg_printf("header: %p\n", HDRP(NEXT_BLKP(bp)));
        // dbg_printf("GET_SIZE(NEXT_BLKP(bp)): %d\n", GET_SIZE(NEXT_BLKP(bp)));
        // dbg_printf("GET_ALLOC(NEXT_BLKP(bp)): %d\n",GET_ALLOC(NEXT_BLKP(bp)));
...
static Bptr extend_heap(size_t words) {
    ...
    dbg_printf("Extended the %p by %zu words.\n", bp, words);  
